...
export default {
  ...
  created() {
    ...
    axios.get('/api/managerSpace/getUserProfile?userName='+localStorage.getItem('loginUser')).then(function (response){
      if(JSON.stringify(response)!== '{}'){

        //localStorage.setItem("allInvoiceList",JSON.stringify(response.data))
        console.log(response.data)
        // _that.formRef.reset();
        _that.form.firstName= response.data.firstName;
        _that.form.lastName= response.data.lastName;
        _that.form.email= response.data.email;
        _that.form.userName= response.data.userName;
        _that.form.password= response.data.password;
  ...
  methods: {
    save(){
      ...
      let createClient = {'firstName':_that.form.firstName,'lastName':_that.form.lastName,'email':_that.form.email,
        'userName':_that.form.userName,'password':_that.form.password,'role':"Manager"}
      axios.defaults.headers.common['Authorization'] = "Bearer "+token
      axios.post('/api/managerSpace/updateProfile',createClient).then(function (response){
